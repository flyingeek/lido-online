<script>
    ...
    import {showGramet, route, grametPosition, ofp} from '../stores';
...
{#if  ($ofp && !$ofp.isFake)}
    {#if $grametStatus !== 'error'}
        <div class="gramet-thumbnail" class:open={$showGramet} class:invisible={$route !== '/map'} use:grametThumbAction={{ofp: $ofp, pos: $grametPosition}} on:click={toggleGramet}>
