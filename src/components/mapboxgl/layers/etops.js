...
function addEtops(data) {
    ...
    const epCircles = [];
    if (ofp.infos['EEP']) {
        epCircles.push(ofp.infos['EEP'].circle(420, 48));
    }
    if (ofp.infos['EXP']) {
        epCircles.push(ofp.infos['EXP'].circle(420, 48));
    }
    if (epCircles.length > 0) {
        addLines(map, `${folder}-ep-circle`, epCircles , affineAndClip, kmlOptions.routeColor, visibility, lineWidth, true);
    }
    if (ofp.infos['raltPoints'].length > 0) {
