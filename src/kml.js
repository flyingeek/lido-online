...
export const KmlGenerator = () => {
  if (!(window['kmlGen'])) {
    ...
    const kmlOptions = kmlDefaultOptions;
    window['kmlGen'].addFolders(
...
export function updateKml(name, value) {
  ...
  if (name.endsWith('-display')) {
      ...
      kmlGen.changeFolderColor(folder, value);
  }else{
      console.err(`ignoring kml update for ${name}`);
  }
}
/**
 ...
 */
export function generateKML(ofp) {
